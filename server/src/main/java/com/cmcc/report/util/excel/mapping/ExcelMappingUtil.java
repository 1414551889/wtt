...
import com.cmcc.report.util.XMLUtil;
...
public class ExcelMappingUtil {
	...
	public static Object createIntanceByruleType(String key)
		...
		if (null != erulemap && null != erulemap.get(key)) {
			...
			if (StringUtil.isNotEmpty(e.getType())) {
	...
	public static String getColumn(String key, Integer index) {
		if (null != columsMap && StringUtil.isNotEmpty(key)) {
			...
			if (StringUtil.isNotEmpty(e.getType())) {
				...
				for(Object obj : objlist){
					...
					for (Integer index : keys) {
						Method declaredMethod = classType.getDeclaredMethod("get"+StringUtil.capitalize(columnMap.get(index)));
					...
					if(null!=other && StringUtil.isNotEmpty(other.getColumn())){
						String fieldName = other.getColumn();
						Method declaredMethod = classType.getDeclaredMethod("get"+StringUtil.capitalize(fieldName));
						Object result = declaredMethod.invoke(obj);
						String [] re = null;
						if(null!=result){
			...
			for(int i=1;i<dataList.size();i++){
				...
				for(int j=0;j<cellarray.size();j++){
					if(keyset.contains(j)){
						...
						Method declaredMethod = clazz.getDeclaredMethod("set"+StringUtil.capitalize(fieldname),field.getType());
