<template lang="html">
  <div>
    ...
    <appPage :total="total" :pageIndex="pageIndex" :pageSize="pageSize" @pageChange="getOnlineTraceBills"></appPage>
  </div>
</template>
...
export default {
            ...
            let btnView = h('Button', {
              ...
              on: {
                click: () => {
                  ...
                  this.$router.push({name: 'traceView', query: {id: params.row.id}})
                }
              }
            }, '查看')
            let btnEdit = h('Button', {
              ...
              on: {
                click: () => {
                  //  ----------------- name = planEdit
                  console.log(params.row.id)
                  this.$router.push({name: 'traceEdit', query: {id: params.row.id}})
                }
              }
            }, '编辑')
              ...
              on: {
                click: () => {
                  let config = {
                    ...
                    onOk: async function() {
                       try {
                        let onlineBillId = params.row.id
                        console.log('onlineBillId = ' + onlineBillId)
                        await api.trace.publishOnlineTraceBill({onlineBillId: onlineBillId})
  ...
  methods: {
    search() {
      this.getOnlineTraceBills(1)
    },
    async getOnlineTraceBills(pageIndex) {
      if (pageIndex) {
      ...
      try {
        ...
        this.loading = true
        // 此处有错误000---------------
        let response = await api.trace.getOnlineTraceBills(params)
