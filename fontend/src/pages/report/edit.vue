<template lang="html">
  <div>
    ...
    <div v-else-if="reportList.length === 0">
      暂无数据
    </div>
    <Collapse v-else v-model="panelId" accordion>
      <Panel v-for="(item, index) in reportList" :key="item.contentId" :name="String(item.contentId)">
        ...
        <p slot="content" class="content">
          ...
          <b class="label">上线计划：</b>{{item.remark}}
        </p>
      </Panel>
    </Collapse>
...
export default {
  ...
  methods: {
    async getOnlineContentDetailData() {
      ...
      try {
        this.loading = true
        let res = await api.report.getOnlineContentByBill({billId})
    ...
    add() {
      let billId = this.$route.query.id
      this.$router.push({name: 'reportAdd', query: {billId:billId}})
    },
    modify(id) {
      this.$router.push({name: 'reportModify', query: {id:id}})
    },
    async publishOnlineBill() {
      let billId = this.$route.query.id
      try {
        let res = await api.report.publishOnlineBill({billId})
        console.log(res)
    ...
    openDefaultPanel() {
      ...
      this.$nextTick(function() {
        this.panelId = String(this.reportList[0].contentId)
      })
    },
    downloadModel() {
      window.location.href = process.env.AXIOS_BASE_URL+'exportOnlineBatchResult';
    },
